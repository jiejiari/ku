...
export default class AdListLoop extends Laya.Script {
	...
	start(adInfos,adID:number,page?:string) {
		...
		for (var i = 0; i < adInfos.length; i++) {
			...
			image.on(Laya.Event.MOUSE_UP, this, function () {
				if (Laya.timer.currTimer - self.startTime <= 200) {
					self.tiaozhuang(adInfos[index], adID,page);
				}
				self.startTime = Laya.timer.currTimer;
			});
	...
	public tiaozhuang(adInfo, positionIdx,page?:string) {
		var _d: any = {
			my_uuid: positionIdx,
		...
		_d.callback = ()=>{
			WXAPI.ald(page+"广告位跳出成功",{
				"path": adInfo.toLinks,
				"appId": adInfo.appid,
				"position": adInfo.position
			})
		}
		WXAPI.ald(page+"广告位跳出",{
			"path": adInfo.toLinks,
			"appId": adInfo.appid,
			"position": adInfo.position
		})
